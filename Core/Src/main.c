...
static void OLED_Update_Task(void *argument)
{
    ...
    for (;;)
    {
        ...
        /* 第2-3行: 速度信息 */
        int32_t left_speed = MotorDriver_GetCurrentSpeed(MOTOR_LEFT) * 100; // 转换为每秒
        int32_t right_speed = MotorDriver_GetCurrentSpeed(MOTOR_RIGHT) * 100;
        SSD1306_printf(2, 0, "V:L%5ld/s", (long)left_speed);
