...
int32_t MotorDriver_PIDControl(MotorID_t motor)
{
    ...
    // 直接读取原始16位计数器，通过int16_t差值自动处理溢出
    // 不经过 MotorDriver_GetEncoderCount()，因为取反会破坏溢出修正
    uint16_t raw_cur;
    int32_t sign;
    if (motor == MOTOR_LEFT) {
        raw_cur = (uint16_t)__HAL_TIM_GET_COUNTER(&htim3);
        sign = 1;
    } else {
        raw_cur = (uint16_t)__HAL_TIM_GET_COUNTER(&htim2);
        sign = -1;
    }

    // 从 last_encoder 还原上次原始计数器值
    uint16_t raw_last;
    if (motor == MOTOR_LEFT) {
        raw_last = (uint16_t)motor_states[motor].last_encoder;
    } else {
        raw_last = (uint16_t)(-motor_states[motor].last_encoder);
    }

    // int16_t差值自动处理65535↔0环绕，无需手动钳位
    int32_t delta = (int32_t)((int16_t)(raw_cur - raw_last)) * sign;

    // 保存当前值（last_encoder存int32_t格式以保持兼容）
    int32_t current_encoder;
    if (motor == MOTOR_LEFT) {
        current_encoder = (int32_t)raw_cur;
    } else {
        current_encoder = -(int32_t)raw_cur;
    }
